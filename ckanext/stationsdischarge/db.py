    ...
    def as_dict(self):
        ...
        for col in self.__table__.columns:
            ...
            if isinstance(val, datetime.datetime):
                val = val.isoformat()
            d[col.name] = val
        # Include telemetry keys
        d["telemetry_keys"] = [k.as_dict() for k in StationTelemetryKey.get_by_station(self.id)]
        return d
...
class HydroDataset(DomainObject, BaseModel):
    ...
    def as_dict(self):
        ...
        for col in self.__table__.columns:
            ...
            d[col.name] = val
        d["stations"] = [
            s.as_dict() for s in HydroDatasetStation.get_by_dataset(self.id)
        ]
        return d
